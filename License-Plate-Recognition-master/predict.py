	...
	def predict(self, car_pic, resize_rate=1):
		...
		for card_index,card_img in enumerate(card_imgs):
			...
			if yello*2 >= card_img_count:
				...
				color = "bw"
			print("车牌颜色（非车牌则显示no）：", color)
			colors.append(color)
			print("蓝、绿、黄、黑、白、总数：", blue, green, yello, black, white, card_img_count)
...
def Output(picture):
	c = CardPredictor()
	c.train_svm()
	r, roi, color = c.predict(picture)
	car_license = ''.join(r)
	return car_license, color

if __name__ == '__main__':
	Output("./test/car4.jpg")