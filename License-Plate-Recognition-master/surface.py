...
class Surface(ttk.Frame):
	...
	def __init__(self, win):
		...
		win.title("Edward ZHAO 19079082D")
		#win.state("zoomed")
		self.pack(fill=tk.BOTH, expand=tk.YES, padx="5", pady="5")
		frame_left.pack(side=LEFT,expand=1,fill=BOTH)
		frame_right1.pack(side=TOP,expand=1,fill=tk.Y)
		frame_right2.pack(side=RIGHT,expand=0)
		ttk.Label(frame_left, text='Raw Image:').pack(anchor="nw") 
		ttk.Label(frame_right1, text='Car License:').grid(column=0, row=0, sticky=tk.W)
		
		from_pic_ctl = ttk.Button(frame_right2, text="Image", width=20, command=self.from_pic)
	...
	def from_pic(self):
		...
		self.pic_path = askopenfilename(title="Select Image", filetypes=[("jpg Image", "*.jpg")])
		if self.pic_path:
